...
public class XfromControl : MonoBehaviour {
	...
	void Start () {
        ...
        /*xLookAt.InitSliderRange(-20, 20, 0);
        yLookAt.InitSliderRange(-20, 20, 0);
        zLookAt.InitSliderRange(-20, 20, 0);*/
        ...
        /*xLookAt.SetSliderListener(XLookAtChanged);
        yLookAt.SetSliderListener(YLookAtChanged);
        zLookAt.SetSliderListener(ZLookAtChanged);*/
    ...
    /*void XLookAtChanged(float v)
    {
        Vector3 p = lookAt.localPosition;
        p.x = v;
        lookAt.localPosition = p;
    }

    void YLookAtChanged(float v)
    {
        Vector3 p = lookAt.localPosition;
        p.y = v;
        lookAt.localPosition = p;
    ...
    {
        if (mSelected != null)
            mSelected.GetComponent<SceneNode>().isSelected = false;
        mSelected = xform;
        xform.GetComponent<SceneNode>().isSelected = true;
    ...
    private void UISetObjectXform(ref Vector3 p, ref Quaternion q)
    {
        ...
        } else
        {
            ...
            qr = mSelected.localRotation * q;

            mSelected.localRotation = qr;


            /*foreach (Transform primitive in mSelected.GetChild(0).GetComponentsInChildren<Transform>())
            {
                primitive.localRotation = q * primitive.localRotation;
            }*/
        }
    }
}